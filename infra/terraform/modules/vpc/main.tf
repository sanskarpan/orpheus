...
locals {
  ...
  # Carve /20 subnets out of the VPC CIDR: public, private (app), database.
  ...
  database_subnets = [for i in range(var.az_count) : cidrsubnet(var.cidr, 4, i + 2 * var.az_count)]
...
resource "aws_subnet" "database" {
  ...
  tags = merge(var.tags, { Name = "${var.name}-db-${local.azs[count.index]}" })
}
